...
@Api(value = "Hospital", tags = {"Hospital Details"})
public class HospitalController extends BaseController {
    ...
    public ResponseEntity<List<HospitalDto>> getAllHospital(@RequestParam(required = false) String search) {
        return new ResponseEntity<>(hospitalService.findHospitals(search), HttpStatus.OK);
    }

    @ApiOperation(value = "Create/Edit a hospital")
    @PostMapping(produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<SuccessResponse> createOrEditHospital(
            @Valid @RequestBody HospitalDto hospitalDto, BindingResult bindingResult) throws QmsException {
        ...
        if(bindingResult.hasFieldErrors()){
            ...
            for(FieldError fieldError: fieldErrors){
                ...
                logger.warn("Hospital create/edit validation ERROR: ------ FieldErrorExists: errorCode: {}, fieldName: {}," +
        ...
        hospitalService.createOrEditHospital(hospitalDto);
        return new ResponseEntity<>(new SuccessResponse("Added Successfully"), HttpStatus.OK);
    }

    @ApiOperation(value = "Get Hospital Details")
    @GetMapping(value = "/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<HospitalDto> getHospital(@PathVariable("id") Long hospitalId) throws QmsException {
        return new ResponseEntity<>(hospitalService.getHospitalDetails(hospitalId), HttpStatus.OK);
    }
}
